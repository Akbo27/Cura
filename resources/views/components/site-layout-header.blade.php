        ...
        <div>
            @auth
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit" class="bg-white text-blue-600 px-4 py-2 rounded-lg  text-uppercase rounded font-semibold hover:bg-gray-200">
                        Log out
                    </button>
                </form>
            @else
                <a href="{{ route('login') }}" class="bg-white text-blue-600 px-4 py-2 rounded-lg text-uppercase rounded font-semibold hover:bg-gray-200">
                    Log in
                </a>
            @endauth
        </div>
